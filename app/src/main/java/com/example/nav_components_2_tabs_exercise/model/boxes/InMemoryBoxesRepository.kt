...
class InMemoryBoxesRepository(
    ...
    private val actionBoxesFlow: Flow<Set<Int>> = combine(reconstructFlow, accountRepository.getAccount()){_,account->
...
//What? Clarification needed.
    ...
    override fun getBoxes(onlyActive: Boolean): Flow<List<Box>> =
        actionBoxesFlow.map { activeIdentifiers ->
            boxes.filter { if (onlyActive) activeIdentifiers.contains(it.id) else true }
        }

    override suspend fun activeBox(box: Box) {
        val account = accountRepository.getAccount().firstOrNull() ?: return
