...
import com.example.nav_components_2_tabs_exercise.Repositories.accountsRepository
...
import com.example.nav_components_2_tabs_exercise.screens.main.tabs.admin.ExpansionStatus.*
...
class AdminViewModel(
    ...
    //--- identifiers should be unique fo each item in the tree to be correctly rendered by DiffUtil
    ...
    private fun getExpansionStatus(
        id: Long,
        hasChildren: Boolean,
        expendedIds: Set<Long>
    ): ExpansionStatus {
        if (!hasChildren) return NOT_EXPANDABLE
        return if (expendedIds.contains(id)) {
            EXPANDED
        } else {
            COLLAPSED
        }
